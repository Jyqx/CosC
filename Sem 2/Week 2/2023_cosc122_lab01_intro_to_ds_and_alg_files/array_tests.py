...
def time_bva_trial(filename, array_size):
    ...
    """
    # ---start student section---
    print('\nRunning trial on Bva array with', filename)
    start_time = time.perf_counter()
    file_size = BitVectorArray(array_size)
...
def main_tests():
    ...
    # Timing tests
    # filename = 'file3.txt'
    # print('Timing', filename, 'with a linear array')
    # time_linear_trial(filename)

    filename = 'file2.txt'
    array_size = 10000
    print('Timing', filename, 'with a bva array')
    time_bva_trial(filename, array_size)
