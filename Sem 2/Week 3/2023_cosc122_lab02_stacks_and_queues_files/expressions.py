...
def evaluate_postfix(expression):
    ...
    for token in tokens:
        if token in OP_PREC:
            param2 = expression_stack.pop()
            param1 = expression_stack.pop()
            result = calculate(token, param1, param2)
            expression_stack.push(result)
        else:
            expression_stack.push(float(token))
...
def infix_to_postfix(infix_expression):
    ...
    for token in tokens:
        if token in OP_PREC:
            if token == "(":
                ...
                while expression_stack.peek() != "(":
                    postfix_expression += expression_stack.pop() + " "
                expression_stack.pop()
            else:
                while not expression_stack.is_empty() and \
                        OP_PREC[expression_stack.peek()] >= OP_PREC[token]:
                    postfix_expression += expression_stack.pop() + " "
                expression_stack.push(token)
        else:
    ...
    while not expression_stack.is_empty():
        postfix_expression += expression_stack.pop() + " "
    return postfix_expression.rstrip()
    # ===end student section===
