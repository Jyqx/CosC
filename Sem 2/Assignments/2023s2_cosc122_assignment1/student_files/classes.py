...
This module provides classes that are to be used to complete Assignment 1. 
These have many careful restrictions placed on them, but do provide a 
sufficient interface to solve the problems given.
"""


from stats import StatCounter

GENE_COMPARISON_TYPE_ERROR = "Gene objects can only be compared to other Gene objects"
...
class Gene(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, Gene):
            raise TypeError(GENE_COMPARISON_TYPE_ERROR)
...
class Genome(object):
    ...
    def __getitem__(self, i):
        StatCounter.increment('accesses')
