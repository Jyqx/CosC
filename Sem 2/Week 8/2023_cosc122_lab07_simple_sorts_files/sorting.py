...
def reversed_selection_sort(file_name):
    """ Loads numbers from the given file and returns them as a reverse-sorted list.
    The numbers are sorted in reverse order using selection sort.
    """
    alist = load_file(file_name)
    n_comps = 0
    for fill_slot in range(len(alist) - 1, 0, -1):
        index_of_min = fill_slot  # Change the variable name
        for location in range(fill_slot - 1, -1, -1):  # Reverse the loop
            n_comps += 1  # Increment the comparison count
    ...
    # Note: you will need to count the comparisons
    print('Reverse Selection sort on {}, {} items'.format(file_name, len(alist)))
    print('  Used {} comparisons.\n'.format(n_comps))
    return alist

def minimum_selection_sort(file_name):
    """ Loads numbers from the given file and returns them as a sorted list.
    The numbers are sorted in ascending order using minimum selection sort.
    """
    alist = load_file(file_name)
    n_comps = 0
    for fill_slot in range(len(alist) - 1):
        index_of_min = fill_slot  # Initialize index_of_min to the current fill_slot
    ...
    # Note: you will need to count the comparisons
    print('Minimum Selection sort on {}, {} items'.format(file_name, len(alist)))
    print('  Used {} comparisons.\n'.format(n_comps))
    return alist
...
def gap_insertion_sort(alist, start, gap):
    ...
    return n_comps  # Return the total number of comparisons

### --- Shell Sorting Functions --- ###
...
def shell_sort2(file_name, gap_list):
    ...
    for gap in gap_list:
        gap_list_used.append(gap)  # build a list of gaps used as we go
...
#     # selection_sort(file_name)
#     # reversed_selection_sort(file_name)
#     # minimum_selection_sort(file_name)
#     # shell_sort(file_name)
#     # print(shell_sort2(file_name, [3]))

# shell_sort2("file15.txt", [5, 2, 1])

file_name = "file17.txt"
insertion_sort(file_name)
shell_sort(file_name)
